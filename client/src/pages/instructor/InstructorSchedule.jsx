...
import { RefreshCw, AlertTriangle, Clock, ChevronLeft, ChevronRight } from "lucide-react";
...
const InstructorSchedule = () => {
  ...
  // Hook 1: Fetch semesters from API on mount
  useEffect(() => {
  ...
  // Hook 2: Generate weeks and set starting week
  useEffect(() => {
  ...
  // Fetch instructor's teaching schedule
  const fetchTeachingSchedule = async () => {
    ...
    try {
      ...
      setErrorAlert("Không thể tải lịch giảng dạy!");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!selectedSemester || !currentWeekStart || !weekDays || weekDays.length === 0) {
    return (
      <div className="flex-1 w-full flex flex-col items-center justify-center p-12 bg-background">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto p-6 space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-muted mt-1 text-sm">
              Tra cứu lịch trình giảng dạy học phần phụ trách theo tuần và học kỳ
            </p>
          </div>
          <button
            onClick={fetchTeachingSchedule}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center p-24 gap-3 bg-surface border border-border rounded-md">
            <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
            <span className="text-muted font-medium animate-pulse">
              Đang nạp thời khóa biểu giảng dạy...
            </span>
          </div>
        ) : (
          /* Visual Timetable Grid using CSS Grid coordinate cells mapping */
          <div className="w-full overflow-x-auto bg-surface border border-border rounded-md p-5 shadow-sm">
            <div className="min-w-[950px] grid grid-cols-[80px_repeat(7,minmax(0,1fr))_80px] grid-rows-[40px_repeat(10,96px)_40px] border-t border-l border-border mt-4 bg-background rounded-md relative overflow-hidden">
              ...
              {/* --- DONG CUOI: FOOTER --- */}
              {/* Cot 1: Nut previous */}
              <div className="border-t border-r border-border p-2 flex items-center justify-center bg-surface rounded-bl-md">
                <button
                  onClick={handlePreviousWeek}
                  className="p-1 hover:bg-background rounded cursor-pointer text-muted hover:text-DEFAULT transition-colors"
                >
              ...
              {/* Cot 9: Nut next */}
              <div className="border-t border-r border-border p-2 flex items-center justify-center bg-surface rounded-br-md">
                <button
                  onClick={handleNextWeek}
                  className="p-1 hover:bg-background rounded cursor-pointer text-muted hover:text-DEFAULT transition-colors"
                >
              ...
              {(scheduleBlocks || []).map((block, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    onClick={() => navigate(`/instructor/classes/${block.classId}`)}
                    ...
                    title="Nhấp để quản lý lớp học phần"
                  >
                    <h4 className="text-xs font-bold leading-tight uppercase text-left line-clamp-2 w-full block">
                      {block.subjectName} ({block.subjectId})
                    </h4>
                    <p className="text-[10px] text-muted font-mono mt-1 block text-left w-full">
                      Phòng: {block.room}
                    </p>
                    <p className="text-[10px] text-muted font-mono mt-1 block text-left w-full flex items-center gap-1">
                      <Clock size={10} />
